...
public class ReviewDAO extends DB.DBContext {
    ...
    /**
     * Retrieves reviews filtered by rating, keyword, and time for a specific
     * product
     *
     * @param productId The product ID to get reviews for
     * @param rating The rating to filter by (null for all ratings)
     * @param keyword The keyword to search in review content (null for no
     * keyword filter)
     * @param timeFilter The time filter ("today", "week", "month", or
     * "all"/null for all time)
    ...
    public List<Review> getReviewsByFilter(int productId, Integer rating, String keyword, String timeFilter) {
        ...
        StringBuilder sql = new StringBuilder(
                "SELECT r.ReviewId, r.ProductVariantId, r.CustomerId, r.Star, r.ReviewContent, "
                + "r.CreatedAt, r.UpdatedAt, r.IsDeleted, "
                + "c.Name as CustomerName, "
                + "p.Name as ProductName, "
                + "pv.Color, "
        ...
        if (timeFilter != null && !timeFilter.trim().isEmpty() && !"all".equalsIgnoreCase(timeFilter)) {
            ...
            LocalDateTime cutoff = calculateTimeCutoff(timeFilter);
            if (cutoff != null) {
    ...
    public boolean createReview(Review review) {
        ...
        try {
            int result = execQuery(sql, params);
            return result > 0;
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public Review getExistingReview(int productVariantId, int customerId) {
        ...
        Object[] params = {productVariantId, customerId};
    ...
    /**
     * Batch load reviews for multiple product variants by a customer Optimizes
     * N+1 query problem when loading reviews for order items
     *
    ...
    public java.util.Map<Integer, Review> getReviewsByProductVariantsAndCustomer(List<Integer> productVariantIds, int customerId) {
        ...
        List<Object> params = new ArrayList<>(productVariantIds);
    ...
    public Review getReviewById(int reviewId, int customerId) {
        ...
        Object[] params = {reviewId, customerId};
    ...
    public boolean addStaffReply(int reviewId, String replyContent, int staffId) {
        String sql = "INSERT INTO Reply (ReviewId, StaffId, ReplyContent, CreatedAt, UpdatedAt, IsDeleted) "
                + "VALUES (?, ?, ?, GETDATE(), GETDATE(), 0)";

        Object[] params = {reviewId, staffId, replyContent};
    ...
    public boolean updateStaffReply(int replyId, String replyContent, int staffId) {
        String sql = "UPDATE Reply SET ReplyContent = ?, UpdatedAt = GETDATE() "
                + "WHERE ReplyId = ? AND StaffId = ? AND IsDeleted = 0";

        Object[] params = {replyContent, replyId, staffId};
