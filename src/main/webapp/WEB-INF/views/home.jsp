...
<head>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .logo { font-size: 2rem; font-weight: bold; text-decoration: none; color: #fff; }
  .search-form { display: flex; gap: 10px; flex: 0 1 400px; }
  .search-input {
    flex: 1; padding: 10px 15px; border: none; border-radius: 25px;
    font-size: 1rem; outline: none; background: #fff; color: #333;
  }
  .search-button {
    background: #000; color: #fff; border: none;
    padding: 10px 20px; border-radius: 25px; cursor: pointer; font-size: 1rem;
    transition: background .3s;
  }
  .search-button:hover { background: #111; }

  /* Categories */
  .categories { background: #fff; padding: 1rem 0; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
  .categories-container { max-width: 1200px; margin: 0 auto; padding: 0 20px; }
  .categories-title { font-size: 1.2rem; margin-bottom: 1rem; color: #333; }
  .categories-list { display: flex; gap: 15px; overflow-x: auto; padding-bottom: 10px; }
  .category-chip {
    display: inline-flex; align-items: center; gap: 10px; padding: 8px 12px;
    border: 2px solid #000; border-radius: 999px; text-decoration: none;
    background: #fff; color: #333; transition: all .2s; white-space: nowrap;
  }
  ...
  .category-thumb {
    ...
    background: #f0f0f0; border: 1px solid rgba(0,0,0,.06);
  }

  /* Products */
  .products { max-width: 1200px; margin: 2rem auto; padding: 0 20px; }
  .section-header {
  ...
  .price-range, .price { font-size: 1.3rem; font-weight: bold; color: #28a745; margin: 10px 0; }

  .view-details-btn {
    display: block; background: #f8f9fa; color: #333; text-align: center;
  ...
  .no-products {
    text-align: center; color: #666; font-size: 1.1rem; margin: 3rem 0; padding: 2rem;
            ...
            <div class="categories-list">
                ...
                <c:forEach var="category" items="${categories}">
                    <a href="${pageContext.request.contextPath}/home?action=category&categoryId=${category.categoryId}"
                       class="category-chip ${param.selectedCategory == category.categoryId ? 'active' : ''}">
                        <img class="category-thumb"
                             src="${empty category.image ? 'https://via.placeholder.com/56?text=Cat' : category.image}"
                             alt="${category.name}"
                             onerror="this.src='https://via.placeholder.com/56?text=Cat'">
