      ...
      <c:choose>
        ...
        <c:when test="${not empty sessionScope.customer}">
          <span class="text-white">
            Hello,
            <c:out value="${sessionScope.customer.name != null
        ...
        <c:otherwise>
          ...
          <a href="${registerUrl}"
             ...
             style="background:linear-gradient(to right,#fbbf24,#f97316);box-shadow:0 2px 4px rgba(0,0,0,.2);transition:transform .2s;">
            Register
          </a>
        </c:otherwise>
      </c:choose>
...
<style>
  ...
  .form-control-header:focus {
    background-color:#1e293b; color:#fff; border-color:#fbbf24;
    box-shadow:0 0 0 .2rem rgba(251,191,36,.25);
  }
  .form-control-header::placeholder { color:#bfdbfe; }
  @media (max-width: 768px){
    .form-control-header{ width: 60vw!important; }
  }
</style>
